...
namespace eshopDL
{
    public class CategoryDL
    {
        ...
        public int SaveCategory(Category category)
        {
            int status;
            using (SqlConnection objConn = new SqlConnection(WebConfigurationManager.ConnectionStrings["eshopConnectionString"].ConnectionString))
                using (SqlCommand objComm = new SqlCommand("INSERT INTO category (name, parentCategoryID, url, imageUrl, sortOrder, pricePercent, webPricePercent, showOnFirstPage, numberOfProducts, firstPageSortOrder, firstPageOrderBy, description, active, categoryBannerID) VALUES (@name, @parentCategoryID, @url, @imageUrl, @sortOrder, @pricePercent, @webPricePercent, @showOnFirstPage, @numberOfProducts, @firstPageSortOrder, @firstPageOrderBy, @description, @active, @categoryBannerID)"))
                {
                    try
                    {
                        objConn.Open();
        ...
        private Category GetCategory(int categoryID, string name)
        {
            ...
            using (SqlConnection objConn = new SqlConnection(WebConfigurationManager.ConnectionStrings["eshopConnectionString"].ConnectionString))
                using (SqlCommand objComm = new SqlCommand("SELECT categoryID, name, parentCategoryID, url, imageUrl, sortOrder, pricePercent, webPricePercent, showOnFirstPage, numberOfProducts, firstPageSortOrder, firstPageOrderBy, description, active, categoryBannerID FROM category"))
                {
                    try
                    {
                        objConn.Open();
                        ...
                        using (SqlDataReader reader = objComm.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                ...
                                category.ParentCategoryID = Convert.IsDBNull(reader[2]) == false ? reader.GetInt32(2) : 0;
                                ...
                                category.CategoryBannerID = reader.GetInt32(14);
                            }
                        }
                    }
                    catch (SqlException ex)
        ...
        public Category GetCategoryByUrl(string categoryUrl)
        {
            ...
            using (SqlConnection objConn = new SqlConnection(WebConfigurationManager.ConnectionStrings["eshopConnectionString"].ConnectionString))
            {
                using (SqlCommand objComm = new SqlCommand("SELECT categoryID, name, parentCategoryID, url, imageUrl, sortOrder, pricePercent, webPricePercent, description, categoryBannerID FROM category WHERE url=@categoryUrl", objConn))
                {
                    objConn.Open();
                    ...
                    using (SqlDataReader reader = objComm.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            category = new Category(reader.GetInt32(0), reader.GetString(1), reader.GetInt32(2), reader.GetString(3), reader.GetString(4), reader.GetInt32(5), reader.GetDouble(6), reader.GetDouble(7), Convert.IsDBNull(reader[8]) == false ? reader.GetString(8) : string.Empty, reader.GetInt32(9));
                        }
                    }
                }
            }
            ...
            using (SqlConnection objConn = new SqlConnection(WebConfigurationManager.ConnectionStrings["eshopConnectionString"].ConnectionString))
            {
                using (SqlCommand objComm = new SqlCommand("category_getSubcategories", objConn))
                {
                    ...
                    using (SqlDataReader reader = objComm.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                            categories.Add(new Category(reader.GetInt32(0), reader.GetString(1), categoryID, reader.GetString(2), string.Empty, 0, 0, 0, string.Empty, 0));
                    }
                }
            }
            return categories;
