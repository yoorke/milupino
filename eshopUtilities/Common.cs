    ...
    public static class Common
    {
        ...
        public static int SendOrderConfirmationMail(string email, string name, Order order)
        {
            ...
            for(int i=0;i<order.Items.Count;i++)
            {
                ...
                body.Append("<td align='center'>" + (i + 1).ToString() + "</td><td>" + "<a href='http://www.milupino.rs/" + order.Items[i].Product.Url + "' style='color:#d3232e'>" + order.Items[i].Product.Name.ToString() + "</a>" + "</td><td align='right'>" + order.Items[i].Quantity.ToString() + "</td><td align='right'>" + string.Format("{0:N2}", order.Items[i].UserPrice) + "</td><td align='right'>" + string.Format("{0:N2}", order.Items[i].Quantity * order.Items[i].UserPrice) + "</td>");
                body.Append("</tr>");
            }
            body.Append("</table>");
            ...
            body.Append("<span style='font-weight:bold;color:#174e87'>Milupino.rs</span>");
            body.Append("</div>");
        ...
        public static int SendUserCreatedConfirmationMail(string email, string password)
        {
            try
            {
                ...
                body.Append("<br/><br/>Vaša online prodavnica ");
                body.Append("<span style='font-weight:bold;color:#d3232e'>Milupino</span>");
                message.Body = body.ToString();
                message.IsBodyHtml = true;
        ...
        public static void SendNewOrderNotification(string orderID, Order order)
        {
            ...
            mail.To.Add(new MailAddress("milupinko@gmail.com"));
            mail.Subject = "Nova porudžbina";
            StringBuilder body = new StringBuilder();
            body.Append("<strong>Nova porudžbina na sajtu www.milupino.rs</strong>");
            body.Append("<br/>");
            body.Append("Porudžbinu možete pogledati na sledećoj stranici: <a href='http://milupino.rs/administrator/order.aspx?orderID=" + orderID + "'>Porudžbine</a>");
